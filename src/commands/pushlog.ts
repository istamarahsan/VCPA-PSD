...
export default class PushlogHandler implements CommandHandler {
	...
	async execute(interaction: CommandInteraction): Promise<void> {
		...
		const pushlogResult = await this.pushlogTarget.push(
			this.toPushData(
				logToPush,
				interaction.options.getString("topic-id")!,
	...
	private toPushData(
		...
		topicId: string,
		recorderName: string,
		mentorDiscordUserIdsInput: string
	): PushlogData {
		return {
			topicId: topicId,
			...
			mentorDiscordUserIds: mentorDiscordUserIdsInput
				.split(" ")
				.map((id) => id.replace("<@", "").replace(">", "")),
			attendees: Array.from(this.arrayGroupBy(sessionLog.events, (event) => event.userId).entries()).map(
				([userId, events]) => {
					return {
						...
						attendanceDuration: Duration.fromMillis(
							events.reduce(
								(duration, event) =>
									duration +
									(event.timeOccurred.toMillis() - sessionLog.timeStarted.toMillis()) *
										(event.type === "Join" ? -1 : 1),
								0
							)
						)
